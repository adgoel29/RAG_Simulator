        ...
        try:
            ...
            if method == "mmr":
                ...
                try:
                    ...
                    raise ImportError("BM25 requires fastembed. Run: pip install fastembed")

                vector_size = len(self.embeddings.embed_query("test"))
                self._make_collection("rag_bm25", vector_size)
                sparse_store = QdrantVectorStore.from_texts(
                    texts=self.chunks,
                    embedding=self.embeddings,
                    sparse_embedding=FastEmbedSparse(model_name="Qdrant/bm25"),
                    client=self.client,
                    collection_name="rag_bm25",
                    retrieval_mode=RetrievalMode.SPARSE,
                )
                return sparse_store.similarity_search(query, k=topk)
